...
export default function Notice() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await axios.put(`${import.meta.env.VITE_API_BASE_URL}/api/notice/updateJOQ12HJK23KOL19/${editId}`, formData);
      } else {
        await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/notice/addJOQ12NHF894JUPO`, formData);
      }
      fetchNotices();
      setMode('list');
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${import.meta.env.VITE_API_BASE_URL}/api/notice/deleteJOQ12ERM0PKO12/${id}`);
      fetchNotices();
    } catch (error) {
