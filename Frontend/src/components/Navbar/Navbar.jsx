...
const Navbar = () => {
  ...
  const [openMastersSemester, setOpenMastersSemester] = useState(null);
  const mastersDropdownRef = useRef(null);

  const toggleMastersSemester = (semesterValue) => {
    setOpenMastersSemester(
      openMastersSemester === semesterValue ? null : semesterValue,
    );
  };
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        latestSemesterRef.current &&
        !latestSemesterRef.current.contains(event.target)
      ) {
        closeAll();
      }
    };
      ...
      <div className={`menu-container ${isMobileMenuOpen ? "active" : ""}`}>
        <div className="logo">
          <Link to="/" onClick={() => setIsMobileMenuOpen(false)}>
            <img src={BupIcon} alt="BUP Logo" className="logo-icon" />
            ICT8 VAULT
          </Link>
        </div>
        ...
        <ul className="menu">
          ...
          <li
            className="dropdown"
            ref={mastersDropdownRef}
            onMouseLeave={() =>
              handleMouseLeave(mastersDropdownRef, () =>
                setIsMastersOpen(false),
              )
            }
          >
            ...
            {isMastersOpen && (
              ...
              >
                {mastersSemesters.map((semester) => (
                  <div key={semester.value} className="semester-item">
                    <button
                      onClick={() => toggleMastersSemester(semester.value)}
                      className="semester-btn"
                    >
                      {semester.name}{" "}
                      {openMastersSemester === semester.value ? "▲" : "▼"}
                    </button>

                    {openMastersSemester === semester.value && (
                      <div className="course-list">
                        {mastersCourseData[semester.value].map((course) => (
                          <Link
                            key={course.code}
          ...
          <li
            ...
            ref={dropdownRef}
            onMouseLeave={() =>
              handleMouseLeave(dropdownRef, () => setIsResourcesOpen(false))
            }
          >
            <div
              className="dropdown-btn"
          ...
          <li>
            <Link to="/more" onClick={() => setIsMobileMenuOpen(false)}>
              More
            </Link>
          </li>
        </ul>
